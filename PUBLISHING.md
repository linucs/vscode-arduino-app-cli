...
## Python stubs (committed artifact)

The Arduino Python type stubs under `stubs/arduino/**` (consumed by **Configure
IntelliSense**) are a **committed, generated artifact** — same model as
`api/openapi.yaml` via [`scripts/fetch-openapi.sh`](scripts/fetch-openapi.sh). They are
generated by the maintainer, committed, and shipped as-is by CI (the release workflow
does **not** regenerate them).

Regenerate when the framework version baked into the App Lab image changes:

1. Edit [`.framework-ref`](.framework-ref) to the new `release/<ver>` tag.
2. `yarn gen:stubs` — clones `arduino/app-bricks-py` (public, anonymous HTTPS, no
   credentials) at that tag and runs `stubgen` (mypy) in a throwaway venv in parse
   mode (no build/install of the framework, so its optional deps aren't needed).
3. Review and **commit** `stubs/` + `.framework-ref` (the new `STUBS_VERSION` records
   the tag).

`yarn check:stubs` regenerates and fails on any uncommitted diff — a quick freshness
guard to run locally before releasing.
...
1. Bump `version` in [`package.json`](package.json) (semver) and update
   [`CHANGELOG.md`](CHANGELOG.md). If the App Lab image's framework changed, bump
   [`.framework-ref`](.framework-ref) and regenerate the stubs (see *Python stubs*
   above).
